...
const {
  ...
  validateSetUpsells,
  validateUpdate,
} = require("../content-types/product/product.validation");
...
module.exports = createCoreController(PRODUCT, ({ strapi }) => ({
  ...
  async toggleStatus(ctx) {
    ...
    const { id, isDraft, type, variations } = await findOneByUuid(
      uuid,
      PRODUCT,
      productFields
    );
    ...
    if (isDraft && type === "variable" && variations.count === 0) {
      throw new BadRequestError(
        "The product must have at least one variation to be published",
        {
  ...
  async import(ctx) {
    const { company, user } = ctx.state;
    const { file, document } = ctx.request.files ?? {};
    const spreadsheet = file ?? document;

    if (!spreadsheet) {
      throw new UnprocessableContentError(["File is required"]);
    }

    const buffer = await fs.readFile(spreadsheet.path);
    const workbook = XLSX.read(buffer, { type: "buffer" });
    const worksheetName = workbook.SheetNames[0];
    const worksheet = workbook.Sheets[worksheetName];

    if (!worksheet) {
      throw new BadRequestError("Invalid file. It has no sheets", {
        key: "product.invalidImportFile",
        path: ctx.request.url,
      });
    }
    ...
    const headers = (rowsWithHeaders[0] ?? []).map((header) =>
      String(header).trim().toLowerCase()
    );
    const requiredHeaders = ["nombre", "sku", "precio"];
    ...
    if (!hasRequiredHeaders) {
      throw new BadRequestError(
        "The file headers must be: Nombre, SKU, Precio",
        {
          key: "product.invalidImportHeaders",
          path: ctx.request.url,
        }
      );
    }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const name = toString(normalizedRow.nombre);
      const sku = toString(normalizedRow.sku);
      const price = parsePrice(normalizedRow.precio);
      ...
      if (price === null) {
        summary.errors.push({
          row: rowIndex,
          sku,
          message: "Precio must be a valid number",
        });
      ...
      if (user?.id) {
        createData.createdByUser = user.id;
      }

      await strapi.entityService.create(PRODUCT, {
        data: createData,
      });
