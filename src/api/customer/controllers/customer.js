...
module.exports = createCoreController( CUSTOMER, ({ strapi }) => ({
    ...
    async toggleStatus(ctx) {
        ...
        const { id, isArchived, credit } = await findOneByUuid( uuid, CUSTOMER, {
            populate : {
                credit : true,
            },
        });
    ...
    async updateDeliveryAddress(ctx) {
        ...
        const conflictingAddress = customerAddresses.find( address => address.name === data.name && address.id != addressId );
    ...
    async setPrimaryDeliveryAddress(ctx) {
        ...
        customerAddresses[ desiredIndex ].isMain = true;
        ...
        return updatedCustomer.deliveryAddresses.find( address => address.id == addressId );
    },
