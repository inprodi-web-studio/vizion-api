...
const defaultSale = (data, preferences, companyInfo) => {
  const {
    addressToShow,
    showLegalName,
    showRFC,
  ...
  const dueDate = dayjs(data.deliveryDate)
    .locale("es-mx")
    .format("MMMM DD, YYYY");
  ...
  const individualDiscounts = data.resume.individualDiscounts ?? 0;
  const globalDiscount = data.resume.globalDiscount?.amount ?? 0;
        ...
        <body>
            <div class="main-container">
                <div class="header-container">
                    ${
                      showLogotype && companyInfo.logotype?.url
                ...
                <div class="meta-container">
                    <div class="quote-from">
                        ...
                        ${
                          showLegalName && companyInfo.fiscalInfo?.legalName
                            ? `<p class="info-text" style="margin-top: 4px">${companyInfo.fiscalInfo?.legalName}</p>`
                            : ""
                        }
                        ${
                          showRFC && companyInfo.fiscalInfo?.rfc
                            ? `<p class="info-text" style="margin-top: 4px">${companyInfo.fiscalInfo?.rfc}</p>`
                            : ""
                        }
                        ${
                          showAddress && street
                            ? `<p class="info-text" style="margin-top: 4px">${`${street} ${extNumber}${
                                intNumber ? ` ${intNumber}` : ""
                              }<br>${suburb}, ${cp}<br>${city}, ${state}<br>${country}`}</p>`
                            : ""
                        }
                        ${
                          showWebsite && companyInfo.website
                            ? `<p class="info-text" style="margin-top: 8px">${companyInfo.website}</p>`
                            : ""
                        }
                    </div>
                    ...
                    <div class="quote-to">
                        ...
                        ${
                          showContactLegalName &&
                          data.customer?.uuid &&
                          data.customer.fiscalInfo?.legalName
                            ? `<p class="info-text" style="margin-top: 4px">${data.customer?.fiscalInfo?.legalName}</p>`
                            : ""
                        }
                        ${
                          showContactRFC &&
                          data.customer?.uuid &&
                          data.customer.fiscalInfo?.rfc
                            ? `<p class="info-text" style="margin-top: 4px">${data.customer?.fiscalInfo?.rfc}</p>`
                            : ""
                        }
                        ${
                          (showContactAddress &&
                            contactAddressToShow === "address" &&
                            data.customer?.mainAddress) ||
                          (contactAddressToShow === "fiscal" &&
                            data.customer?.fiscalInfo?.address) ||
                          data.lead?.mainAddress
                            ? `<p class="info-text" style="margin-top: 4px">${`${
                                contactStreet ?? ""
                              } ${
                                contactExtNumber ? `#${contactExtNumber}` : ""
                              } ${contactIntNumber ?? ""}
                        ...
                        <div class="address-container">
                            ...
                            <div class="delivery-info">
                                ...
                                <p class="info-text" style="color: #868E96">
                                    ${`${deliveryStreet} ${deliveryExtNumber}${
                                      deliveryIntNumber
                                        ? ` ${deliveryIntNumber}`
                                        : ""
                                    }<br>${deliverySuburb}, ${deliveryCp}<br>${deliveryCity}, ${deliveryState}<br>${deliveryCountry}`}
                                </p>
                            </div>
                        </div>
                    </div>`
            ...
            <div class="specs-container" style="margin-top: 30px">
                ${
                  data.deliveryTime === -1 && data.deliveryTime
                    ? ""
                    : `<div class="deliver-container">
                    <p class="label">Tiempo de Entrega:</p>
                    <p class="info-text">${data.deliveryTime} ${
                        data.deliveryTime === 1 ? "Día" : "Días"
                      }</p>
                </div>`
                }
                <div class="scheme-container">
                    ...
                    <p class="info-text">${
                      schemeDictionary[data.paymentScheme]
                    }</p>
                </div>
            </div>
                  ...
                  .map((item) => {
                    ...
                    return `<div class="body">
                        <div style="grid-column-end: span 6">
                            <div class="product-container">
                                ...
                                <div class="product-info">
                                    ...
                                    <p style="font-size: 10px; font-weight: 400; color: #868E96">${
                                      item.product.sku
                                        ? item.product.sku
                                        : item.variation.sku
                                    }</p>
                            ...
                            ${
                              item.product.description
                                ? `<div class="description" style="margin-top: 10px">${item.product.description.replaceAll(
                                    '"',
                                    "'"
                                  )}</div>`
                                : ""
                        ...
                        <div style="width: 100%; grid-column-end: span 2;">
                            <div class="info-text" style="width: 100%; text-align: center;">${
                              item.quantity
                            } ${
                      item.package
                        ? item.package.unity.name
                        : item.unity?.name ?? "Piezas"
                    }</div>
                            ${
                              item.package
                                ? `
                                <div class="label" style="text-align: center; line-height: 150%; font-size: 10px;">Equivalente a ${Number(
                                  item.package.realConversion * item.quantity
                                ).toLocaleString()} ${item.unity?.name}</div>
                            `
                                : ""
                            }
                        </div>
                        ...
                        <div style="grid-column-end: span 2">
                            ...
                            ${
                              item.discount?.amount
                                ? `<div class="discount" style="margin-top: 2px;">${formatCurrency(
                                    item.price
                                  )}</div>`
                                : ""
                            }
                        ...
                        <div class="info-text" style="grid-column-end: span 2">${formatCurrency(
                ...
                <div class="resume-container" style="margin-top: 20px">
                    ...
                    <div class="resume-table" style="width: 45%; min-width: 45%">
                        <div class="item">
                            ...
                            <p class="number">${formatCurrency(
                              data.resume.subtotal
                            )}</p>
                        </div>
                        ...
                        ${
                          data.resume.individualDiscounts
                            ? `
                            <div class="item">
                        ...
                        ${
                          data.resume.globalDiscount &&
                          data.resume.globalDiscount?.amount > 0
                            ? `
                            <div class="item">
                                <p class="label">Descuento Global</p>
                                <p class="number">- ${formatCurrency(
                                  globalDiscount
                                )} (${data.resume.globalDiscount.percent}%)</p>
                            </div>
                        ...
                        ${
                          data.resume.globalDiscount?.amount ||
                          data.resume.individualDiscounts
                            ? `
                            <div class="item">
